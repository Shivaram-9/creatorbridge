export default function CreatorTemplate18({ data }) {
  return (
    <div className="p-10 font-sans text-slate-800 dark:text-white bg-stone-50 min-h-[297mm]">
      <header className="border-b-4 border-violet-500 pb-6 mb-8 text-center">
        <h1 className="text-4xl font-bold uppercase tracking-widest mb-2 text-violet-900">{data.personal.name || "YOUR NAME"}</h1>
      ...
      {data.personal.summary && (
        <section className="mb-10 text-center max-w-3xl mx-auto">
          <p className="text-lg leading-relaxed text-slate-600 italic border-l-4 border-violet-300 pl-4 text-left">
            {data.personal.summary}
          </p>
        </section>
      )}
      ...
      <div className="flex gap-10">
        <div className="w-2/3 space-y-10">
          {data.experience.length > 0 && (
            <section>
              <h3 className="text-lg font-bold uppercase tracking-widest text-violet-800 border-b-2 border-slate-200 pb-2 mb-6">Experience</h3>
          ...
          {data.projects.length > 0 && (
            <section>
              <h3 className="text-lg font-bold uppercase tracking-widest text-violet-800 border-b-2 border-slate-200 pb-2 mb-6">Projects</h3>
        ...
        <div className="w-1/3 space-y-10">
          {data.skills.length > 0 && (
            <section>
              <h3 className="text-lg font-bold uppercase tracking-widest text-violet-800 border-b-2 border-slate-200 pb-2 mb-6">Skills</h3>
              <div className="flex flex-wrap gap-2">
                {data.skills.map(skill => (
                  <span key={skill} className="px-3 py-1 bg-violet-50 text-violet-700 text-xs font-bold rounded-full border border-violet-100">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {data.education.length > 0 && (
            <section>
              <h3 className="text-lg font-bold uppercase tracking-widest text-violet-800 border-b-2 border-slate-200 pb-2 mb-6">Education</h3>
