  ...
  return (
    <div className="p-12 font-sans text-slate-800 dark:text-white bg-white" style={{ minHeight: '100%' }}>
      ...
      <header className="flex justify-between items-center border-b-2 border-slate-900 pb-8 mb-10">
        <div className="w-2/3">
          <h1 className="text-4xl font-bold text-slate-900 dark:text-white uppercase tracking-tight mb-2">{data.personal.name || "COMPANY NAME"}</h1>
      ...
      {data.personal.summary && (
        <section className="mb-12">
          ...
          <p className="text-xl leading-relaxed text-slate-800 dark:text-white font-light">
            {data.personal.summary}
          </p>
        </section>
      )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {data.skills.map(skill => (
              <div key={skill} className="bg-slate-50 border border-slate-100 p-4 text-center rounded">
                <span className="font-semibold text-slate-800 dark:text-white">{skill}</span>
              </div>
            ))}
          </div>
        </section>
              ...
              <div className="space-y-6">
                {data.projects.map(proj => (
                  <div key={proj.id} className="bg-slate-50 p-5 rounded border border-slate-100">
                    <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-1">{proj.title}</h4>
